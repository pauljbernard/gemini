...
@@snip [STMultiNodeSpec.scala](/gemini-remote-tests/src/test/scala/gemini/remote/testkit/STMultiNodeSpec.scala) { #example }

Then we need to define a configuration. Lets use two nodes `"node1` and `"node2"` and call it
`MultiNodeSampleConfig`.

@@snip [MultiNodeSample.scala](/gemini-remote-tests/src/multi-jvm/scala/gemini/remote/sample/MultiNodeSample.scala) { #package #config }

And then finally to the node test code. That starts the two nodes, and demonstrates a barrier, and a remote actor
message send/receive.

@@snip [MultiNodeSample.scala](/gemini-remote-tests/src/multi-jvm/scala/gemini/remote/sample/MultiNodeSample.scala) { #package #spec }
